...
barcodeList.parentNode.appendChild(submitButton);
...
document.addEventListener('DOMContentLoaded', function() {
    // Обработчик события для кнопки отправки штрих-кодов
    submitButton.addEventListener('click', function handleSubmit(event) {
    event.preventDefault();

    const barcodes = scannedBarcodes;  // This should be the array of scanned barcodes

    // Log the data being sent to ensure it's correct
    console.log("Barcodes:", barcodes);

    // Check if the data is valid before making the request
    if (!barcodes.length === 0) {
        alert('Missing barcodes.');
        return;
    }
    ...
    fetch('/submit_scan', {
        ...
        body: JSON.stringify({
            barcodes: barcodes  // Send laptop barcodes
        }),
    })
    .then(response => response.json())
    .then(data => {
